...
fn test_attr_vals_byte_count() {
    ...
    // Attr 45 (SPACE_USED): 8 bytes
    // Attr 47 (TIME_ACCESS): 12 bytes (i64 sec + u32 nsec)
    ...
    // Total: 4+8+8+16+8+4+4+8+8+8+8+12+12+12+8 = 128 bytes
    //
    // Note: The actual encoded size depends on which attributes are requested
    // and the specific encoding. This test verifies the calculation is consistent.
