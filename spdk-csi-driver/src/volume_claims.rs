...
pub struct VolumeClaims {
    inner: Mutex<HashMap<String, &'static str>>,
}
...
impl VolumeClaims {
    ...
    pub fn try_claim<'a>(&'a self, volume_id: &str, op: &'static str) -> Option<VolumeClaim<'a>> {
        ...
        inner.insert(volume_id.to_string(), op);
        Some(VolumeClaim { claims: self, volume_id: volume_id.to_string() })
    }

    /// Which operation currently holds `volume_id`, if any.
    pub fn holder(&self, volume_id: &str) -> Option<&'static str> {
        self.inner
            .lock()
...
mod tests {
    ...
    fn claim_is_exclusive_per_volume_and_released_on_drop() {
        ...
        assert_eq!(claims.holder("vol1"), Some(OP_CATCHUP));

        // A different volume is independent.
        let other = claims.try_claim("vol2", OP_CUTOVER).expect("other volume");
        assert_eq!(claims.holder("vol2"), Some(OP_CUTOVER));
        drop(other);

        drop(held);
        assert_eq!(claims.holder("vol1"), None);
