...
pub struct FlintLeanWorkspaceSpec {
    ...
    /// NOT READ under CSI delivery: a workspace's syncer gets its
    /// credential from `identity` (the broker by default; `static` reads
    /// the pod's `nodePublishSecretRef`). This named the Secret the
    /// sidecar-injection webhook stamped onto the syncer before v1.45.0,
    /// and is kept so an old CR still applies. The OPERATOR never used
    /// it — bucket-admin ops run under the operator principal.
    #[serde(default, skip_serializing_if = "Option::is_none")]
