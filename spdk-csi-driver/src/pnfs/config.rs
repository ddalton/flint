...
fn default_tier_heartbeat() -> u64 {
    // 10s x 6 misses = a 60s lease. Lease exclusion trades THREE things
    // against each other — renewal cost, takeover latency, and
    // tolerance of transient failures — and you only get two. This
    // setting maximizes the last two, which is also the one that costs
    // the most PUTs: one per 10s per live hub forever, independent of
    // any client I/O (30 PUT/s and ~2.6M object versions a day at 300
    // live hubs).
    //
    // 30 x 4 was tried and REVERTED before release: it is a third of
    // the requests for the same failure tolerance, but it doubles
    // takeover latency after an unclean death, and unlike the fleet
    // changes it shipped beside, nothing had measured it on a cluster.
    // A timing change to the fencing lease is not something to ship on
    // a unit test — the last cluster measurement of a foreign takeover
    // was 80s at 10 x 6, and there is no matching number for 30 x 4.
    // Both remain per-share knobs.
    10
}
fn default_tier_lease_misses() -> u32 {
    // 6 x 10s = a 60s lease. See `default_tier_heartbeat`.
    6
}
...
mod tests {
    ...
    fn tier_section_parses_with_defaults() {
        ...
        // 10 x 6 = a 60s lease. Pinned as a PAIR, because the product
