...
struct EpochBody {
    holder_id: String,
    epoch: u64,
    /// Salt so every renew changes the object's ETag (the CAS token);
    /// liveness is still judged on S3's Last-Modified, not this.
    renewed_unix: u64,
}
...
impl S3Store {
    ...
    async fn bootstrap_lifecycle(&self, prefix: &str, report: &mut BootstrapReport) {
        ...
        let mut rules: Vec<LifecycleRule> = match &existing {
            ...
            Err(e) => {
                let mapped = format!("{}", e);
                if mapped.contains("NoSuchLifecycleConfiguration") {
                    Vec::new()
                } else {
