...
//!
//! The gap this closes. `put_file` is a LIVE write — the object lands
//! at `files/<path>`, the real key, and the syncer integrates it at
//! step 1 of the next barrier (default cadence 60 s). Nothing gates it
//! and nobody approves it, so there was no way to say "I edited this,
//! keep it for me, don't show it to anyone yet." Keeping it in the
//! browser is not an answer: the requirement is that it survives
//! closing the laptop.
//!
...
//! refused `DraftStale` if it moved).
//!
...
//!
//! The verbs are methods on [`Workspace`]; `http.rs` serves them
//! over HTTP and carries no logic of its own.
...
/// A saved draft, read back.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Draft {
    /// The body object's entity-tag.
    pub etag: String,
    /// The recorded base, so the caller can compare it against a fresh
    /// read of `files/<path>` itself.
    pub base_etag: Option<String>,
    /// The body exists but its meta never landed. Readable — the bytes
    /// are the user's work and withholding them helps nobody — but
    /// promote will refuse it until it is re-saved.
    pub incomplete: bool,
    pub body: Bytes,
}
...
fn stamps(author: &str) -> GenerationStamps {
    GenerationStamps {
        ...
        // matches the HITL write, and for the same reason: this is
        // the second writer, not the holder.
        epoch: 0,
...
fn check_user_and_path(user: &str, path: &str) -> Result<(), VerbError> {
    if !user_ok(user) {
        return Err(VerbError::BadUser(user.to_string()));
    }
    if !path_ok(path) {
...
impl Workspace {
    /// Save a draft, overwriting any previous draft of the same path by
    /// the same user. Returns the body's entity-tag.
    ///
    /// Unconditional on purpose (see the module note): the save is the
    /// durability promise, and a save that can be refused is not one.
    /// `base_etag` is the etag of `files/<path>` as the editor read
    /// it, or `None` if the editor claims the file did not exist.
    pub async fn put_draft(
        ...
        let author = author.unwrap_or(user).to_string();
        // Recorded VERBATIM. An entity-tag is an opaque token, and the
        // one thing this must never do is invent a second normalisation
        // rule — `workspace::normalize_etag` is the crate's, applied at
        // COMPARISON, and the first version of this line stripped the
        // quotes here instead, so a stored bare tag never matched the
        // store's quoted one and every promote came back stale.
        let base_etag = base_etag.map(|e| e.trim().to_string());

        // Body FIRST. A crash here leaves nothing; a crash after it
        // leaves an incomplete draft, which promote names and refuses.
        //
        // `Unconditional` against that variant's own warning, and this
        // is the justification. A save must never be refused — that is
        // the whole durability promise — so a second tab cannot be
        // handed a 412 here, and last-write-wins is what an editor's
        // autosave means anyway. The distinction the warning protects
        // is not lost, only MOVED: the meta records the resulting
        // `body_etag`, promote guards the copy source with it, and a
        // promote racing a re-save is refused `DraftMoved` rather than
        // publishing bytes it never saw.
    ...
    /// The saved bytes, with the recorded base so the caller can
    /// compare it against a fresh read of `files/<path>` itself.
    pub async fn get_draft(&self, user: &str, path: &str) -> Result<Draft, VerbError> {
        check_user_and_path(user, path)?;
        let cfg = self.config();
        let meta = load_meta(self.store().as_ref(), cfg, user, path).await?;
        match self.store().get_whole(&cfg.draft_body_key(user, path), None).await {
            Ok((m, body)) => Ok(Draft {
                etag: m.etag,
                base_etag: meta.as_ref().and_then(|m| m.base_etag.clone()),
                incomplete: meta.is_none(),
                body,
            }),
            Err(StoreError::NotFound(_)) => Err(VerbError::NoDraft(format!("{user}: {path}"))),
            Err(e) => Err(e.into()),
        }
    }
    ...
    /// The resume view.
    ///
    /// One HEAD of `files/<path>` per draft to fill `stale`. That is
    /// N+1 requests for N drafts, taken deliberately: the whole point
    /// of the listing is the user coming back days later, and "which
    /// of these can still be published" is the question they have.
    pub async fn list_drafts(&self, user: &str) -> Result<Vec<DraftRow>, VerbError> {
        if !user_ok(user) {
            return Err(VerbError::BadUser(user.to_string()));
        }
        let cfg = self.config();
    ...
    /// Discard a draft.
    ///
    /// Meta first: the window leaves the one incomplete shape the rest
    /// of this module already handles, rather than a meta pointing at
    ...
    pub async fn delete_draft(&self, user: &str, path: &str) -> Result<(), VerbError> {
        ...
        self.store().delete(&cfg.draft_body_key(user, path)).await?;
        Ok(())
    }
    ...
    pub async fn promote_draft(
        ...
        let meta = match load_meta(self.store().as_ref(), cfg, user, path).await? {
            Some(m) => m,
            ...
            None => {
                return Err(VerbError::NoDraft(format!(
            ...
            .await
        {
            ...
            Err(StoreError::NotFound(_)) => {
                return Err(VerbError::NoDraft(format!(
                    "{user}: {path} has meta but no body — re-save the draft"
                )))
            }
            Err(e) => return Err(e.into()),
        };
        ...
        // a second promote is refused `DraftStale` rather than
        // republishing — the failure degrades to a refusal, never to a
        // double write.
        let _ = self.store().delete(&cfg.draft_meta_key(user, path)).await;
        let _ = self.store().delete(&cfg.draft_body_key(user, path)).await;
