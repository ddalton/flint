#!/bin/bash
# H1f: the known-bad world must go red, its probe must fire, the fixed
# four-barrier world must hold; then the earlier deciders on this module,
# the 132-run gate, the trace replays, the crash world at OrphanTrack=TRUE.
...
BAD=0
expect LeanBarrierLeaseSupersedeDropsBase fail
expect LeanProbeBaseRestored fail
expect LeanBarrierLeaseOrphanConverges hold
echo "THREEDONE bad=$BAD"
[ $BAD -eq 0 ] || { echo "stopping before the rest"; exit 1; }
