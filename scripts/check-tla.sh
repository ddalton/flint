...
#
# Two hundred and forty-six runs, ALL required.
#
...
#
#   71 FlintReplication    33 FlintComposition    24 ForgeSync
...
mutation_run ForgeSync ForgeSyncReclaimUnlinksServing.cfg "forge-sync RECLAIM-UNLINK PLACEMENT (ReclaimUnlinks=TRUE with ReclaimWhileServing=TRUE, 72s: the SALVAGED direction 4 -- unlink what the reclaim drops instead of retaining it -- taken OUTSIDE the Importing->Serving window. The strict placement HOLDS at 86,039,237 distinct states, depth 62, 1h 33min, and is deliberately not in this gate; THIS is what says the window is load-bearing rather than the unlinking being universally safe. It violates Inv_AckedIsDurable, not LandedPackComplete: outside the window, unlinking destroys a pack an ACKED push needed. The two arms differ in exactly one constant)" "Inv_AckedIsDurable"
mutation_run ForgeSync ForgeSyncReclaimAtRestore.cfg "forge-sync RECLAIM REFUTED (ReclaimAtRestore=TRUE with RestoreLosesQueue=TRUE, ~4min: 'direction 4' -- take the relaxed supersede rule ONLY between the restore and the first served hook, where no push can arrive. It loses anyway, and the reason is the whole finding: the window stops pushes ARRIVING, not the client RETRYING afterwards. The reclaim retains a refused push's pack; the client retries; the retry reuses the pack NAME because pack names are many-to-one on real git (identical content, identical checksum, measured 2026-09-08); Listing == localPacks \\ retained excludes it; the push lands with its objects in nothing the snapshot names. This run was VACUOUS until 2026-09-08 -- FoldPlan allowed a fold in the window but FoldInit/FoldComplete did not, so it could never commit, and it was caught only because a neighbouring mutation returned byte-identical state counts)" "Inv_LandedPackComplete"
mutation_run ForgeSync ForgeSyncReclaimBySet.cfg "forge-sync FREE-RECLAIM REFUTED (ReclaimBySet=TRUE, ~4min: the reclaim without the repack -- the coverer is the KEPT SET rather than a fresh pack, which measurement showed collects 100% of the residue at every base cadence while building nothing and uploading nothing. It is the version anyone would actually want to build, and it dies for exactly the same reason as the placement above. Keep both: this one pins the CHEAP form, which is the tempting one)" "Inv_LandedPackComplete"
...
mutation_run FlintCsiMount FlintCsiMountReleaseFirst.cfg "multi-cluster mutation (DrainBeforeRelease=FALSE: the pod exited CLEANLY, but the lease went before the final publish, the next cluster superseded the prefix in the gap, and the departing syncer's conditional write is refused — the agent's last work is gone and nothing says so, because the pod is Terminated, the volume is unpublished and the bucket is simply older than it should be)" "Inv_NoLostDrain"
