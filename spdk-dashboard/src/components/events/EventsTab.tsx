...
import { useEvents, WINDOW_TARGET_MS } from '../../hooks/useEvents';
import type { EngineEvent, EventCategory, HotRejoinWindow } from '../../hooks/useEvents';

// 2c: the after-the-fact surfaces for what the 2a indicator cannot show
// live — completed hot-rejoin windows (with step timings vs the 2s target)
// and the engine event timeline.
...
function WindowRow({ w }: { w: HotRejoinWindow }) {
...
function EventRow({ e }: { e: EngineEvent }) {
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {['Time', 'Volume', 'Node', 'Window', 'Path', 'Steps'].map((h) => (
                    <th
                      key={h}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {windows.map((w, i) => (
                  <WindowRow key={`${w.volume}-${w.timestamp}-${i}`} w={w} />
                ))}
              </tbody>
            </table>
          </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {visible.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {visible.map((e, i) => (
              <EventRow key={`${e.volume}-${e.timestamp}-${e.reason}-${i}`} e={e} />
            ))}
          </div>
        )}
      </div>
