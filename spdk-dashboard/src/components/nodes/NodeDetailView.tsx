...
interface NodeDetailViewProps {
  node: string;
  // Fleet rollup row (/api/nodes) — the header's numbers. The expanded
  // body renders per-disk/per-volume detail from the aggregate below.
  summary: FleetNode;
  nodeDisks: Disk[];
  nodeVolumes: Volume[];
...
export const NodeDetailView: React.FC<NodeDetailViewProps> = ({
  ...
  const capacityGb = Math.round(summary.capacity_gb);
  const allocatedGb = Math.round(summary.allocated_gb);
  const utilizationPct = summary.capacity_gb > 0
    ? Math.round((summary.allocated_gb / summary.capacity_gb) * 100)
    : 0;
  const healthStyle = nodeHealthStyle(summary.health);

  return (
    <div className="bg-white rounded-lg shadow">
      {/* Compact summary row — the whole card collapsed to one line; the
          former stat-card grid lives here as inline figures. */}
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        ...
        <span
          className={`px-2 py-0.5 text-xs rounded-full border flex-shrink-0 ${healthStyle.chip}`}
          title={healthStyle.description}
        >
          {healthStyle.label}
        </span>
        ...
        <span className="ml-auto flex items-center gap-4 text-sm text-gray-600 flex-shrink-0">
          <span className="flex items-center gap-1" title="Healthy NVMe disks">
            <HardDrive className="w-4 h-4 text-gray-400" />
            {summary.disks_healthy}/{summary.disks_total}
          </span>
          <span className="flex items-center gap-1" title="Volume replicas on this node">
            <Database className="w-4 h-4 text-gray-400" />
            {volumeFilter && volumeFilter !== 'all'
              ? <>{filteredVolumeCount}<span className="text-gray-400">/{summary.volumes_total}</span></>
              : summary.volumes_total}
          </span>
          <span className="flex items-center gap-1" title="Local NVMe volumes">
            <Zap className="w-4 h-4 text-gray-400" />
            {volumeFilter && volumeFilter !== 'all' ? filteredLocalNVMeCount : summary.local_nvme_volumes}
          </span>
          <span className="hidden md:flex items-center gap-1" title="Capacity used">
            <Activity className="w-4 h-4 text-gray-400" />
            {utilizationPct}%
            <span className="text-gray-400">· {allocatedGb}/{capacityGb}GB</span>
          </span>
        </span>
      </button>
      ...
      <p className="text-sm text-gray-600">
        {capacityGb}GB total • {allocatedGb}GB allocated • {capacityGb - allocatedGb}GB free
      </p>
                ...
                return (
                  <React.Fragment key={disk.id}>
                    <tr className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        {/* allocated_space and capacity_gb are both GB
                            (capacity is bytes) — see useDashboardData. */}
                        <div className="text-sm">
                          <div className="flex items-center gap-2 mb-1">
                            <ProgressBar
                              value={disk.capacity_gb > 0 ? (disk.allocated_space / disk.capacity_gb) * 100 : 0}
                              label={`${disk.id} allocation`}
                              valueText={`${disk.capacity_gb > 0 ? Math.round((disk.allocated_space / disk.capacity_gb) * 100) : 0}% allocated`}
                              className="w-20"
                            />
                            <span className="text-xs text-gray-600">
                              {disk.capacity_gb > 0 ? Math.round((disk.allocated_space / disk.capacity_gb) * 100) : 0}%
                            </span>
                          </div>
                          <div className="text-xs text-gray-500">
                            {Math.round(disk.allocated_space)}GB / {disk.capacity_gb}GB used
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
