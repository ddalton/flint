...
image:https://github.com/ddalton/flint/actions/workflows/rust-ci.yml/badge.svg?branch=main[rust-ci,link=https://github.com/ddalton/flint/actions/workflows/rust-ci.yml]
image:https://github.com/ddalton/flint/actions/workflows/dashboard-ci.yml/badge.svg?branch=main[dashboard-ci,link=https://github.com/ddalton/flint/actions/workflows/dashboard-ci.yml]
image:https://github.com/ddalton/flint/actions/workflows/dashboard-api-spec.yml/badge.svg?branch=main[dashboard-api-spec,link=https://github.com/ddalton/flint/actions/workflows/dashboard-api-spec.yml]
image:https://github.com/ddalton/flint/actions/workflows/formal-lean.yml/badge.svg?branch=main[formal-lean,link=https://github.com/ddalton/flint/actions/workflows/formal-lean.yml]
image:https://github.com/ddalton/flint/actions/workflows/formal-hub.yml/badge.svg?branch=main[formal-hub,link=https://github.com/ddalton/flint/actions/workflows/formal-hub.yml]

// `?branch=main` is deliberate. Without it a badge reports the most
// recent run of the workflow on ANY ref, so a red run from a fork's
// pull request shows as this project being broken — and, worse, a
// green one from a stale branch shows main as healthy when it is not.
// The badge is only worth putting in a README if it answers "is main
// good right now".
//
// rust-ci carries five jobs: the hub's lib+integration suites, the
// flint-store crate, flint-lean, flint-forge, and the cross-uid
// authorization drill against a real kernel NFS mount.
//
// `formal-lean` and `formal-hub` are the TLA+ gates, split because
// GitHub path filters are WORKFLOW-level: one file holding both ran the
// hub's 250-run gate on every lean-only edit. A green badge there says
// the models still
// CHECK OUT — every strict run holds and every must-fail mutation still
// fails. It does NOT say the models still describe the code; nothing
// automated can, and reading it that way is how an abstraction becomes
// the bug.
...
TIP: Looking for a shared POSIX filesystem without any of this machinery?
**Flint-lite** is a separate, minimal chart (`flint-lite-chart/`): one pod
on any CSI driver's RWO PVC serving real NFSv4.2 (locks, close-to-open
coherence), an optional S3 cold tier, and nothing to install on consumer
clusters — no SPDK, no dedicated hardware, no CRDs. See
`flint-lite-chart/README.md` and `docs/flint-lite.md`. For a FLEET of
them (bucket-per-tenant workspaces), `flint-lite-operator-chart/` adds
one `FlintShare` custom resource per volume — see
`docs/flint-lite-operator.md`. To reach those hubs' HTTP file APIs from
outside the cluster, the same chart can run `flint-hub-gateway`: one
door that resolves a project id to its share, wakes it if it is parked,
and proxies the file routes — see `docs/flint-hub-gateway.md`.
Everything below this line concerns the full CSI driver.
