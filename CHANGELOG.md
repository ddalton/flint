...
- **lean: a gateway PUT to a name ending `.flint-sync-tmp` was acked and then
  ...
  skips that suffix at every depth (the consume's own temp sibling), and the
  gateway accepted it: the consume materialised the file, no scan could see
  it, the citation repair cited it as a first absence, and the next barrier
  classified it a delete and collected the object — acked bytes gone with
  only a trace line, no race needed. The gateway now refuses the name
  (`bad-path`), and a citation or inbox entry carrying it is surfaced with a
  `consume-refused-containment` record and never materialised. `AGENTS.md`
  names the reserved suffix.
- **lean: a holder deposed after its pointer CAS landed still ran its
  deletes, and could take an etag the successor had just re-cited** (C2;
  L-104). The commit section read the cell before the CAS and then every 200
  deletes, and S3's DeleteObject carries no epoch; a stall past the 60 s
  deposal threshold inside step 6 — a GC pause, a partition — left the
  successor's citation dangling and the straggler's release answering Ok.
  The collector now renews before a delete whenever its last cell WRITE is
  older than `renew_within_secs` (20 s; a deposal needs the token still for
  60 s), which fences; the every-200 renewal and its store-clock freshness
  test (`lease-4`) are gone with it. The model's `GCDelete` was atomically
  fenced and judged "still referenced" against the live manifest, so no
  world could see it: `GCFencePerDelete` and the writer's own installed
  document are the arms, `LeanBarrierLeaseStragglerGC` finds the loss in 17
  steps and its control holds. What remains is a stall between the renew
  and the DELETE it licenses (`lean/SAFETY.md` §4.13).
- **lean: on a store without a conditional DELETE (MinIO, Ozone) a peer's
  published delete was resurrected, and two writers flapped the path
  forever** (H1; L-105). The collector gives way on such a store and leaves
  the retired object at the key; the other writer's consume found an object
  there and called its queued tombstone superseded — the L-1 rule looked at
  the key, not at what the delete retired — so its baseline kept the path,
  its merge base did not, the path became a citation-repair candidate and
  its next commit re-cited the deleted generation. A tombstone now carries
  the etag the merge base cited (`ForeignChange.retired`), and only a
  DIFFERENT object at the key supersedes it; the retired generation itself
  is what the tombstone is about, and it applies. The model gained
  `TombstoneNamesRetired`, the state-anchored `Inv_NoDeleteResurrected` (a
  published, uncontested delete stays published until a write of the path),
  and the world that fails without the fix in 18 steps. `SAFETY.md` §2 no
  longer calls the leak only "a cost".
- **lean: the untracked sweep's inbox entry outlived the citation it was
  judged against, and a published delete came back through it** (H1b;
  L-106; found by the model the same evening, once `Inv_NoDeleteResurrected`
  reached the sweep's world). The sweep (finding 10) tracks an object at a
  cited key whose etag the manifest does not cite — a live writer's upload
  still in flight qualifies once its grace elapses, and a long step 3 is
  enough. The uploader then commits that very generation, the entry stays
  (the window clear removes only what a barrier CONSUMED), the uploader's
  agent deletes the file and the delete is published; the other writer's
  consume then adopted the entry — or had adopted it while it was still
  pending — and its citation repair re-cited the deleted generation on every
  tree. Three rules now: the entry carries the citation the sweep judged the
  object against (`InboxEntry::cited`) and a consume drops it once that
  citation has moved; an adoption made while the entry stood is provisional
  (`BaselineEntry::judged`) and its repair is withheld once the citation
  moves, with the path queued as a tombstone where the manifest now cites
  nothing; and a leaked object — one the manifest does not cite and no
  surviving entry names — supersedes no tombstone, so the delete reaches the
  tree (before this, a leaked generation the tree had never integrated kept
  a clean stale copy there forever, which no invariant saw: `Inv_TreesConverged`
  is new). Each rule has the test that fails with it reverted; the model
  has `OrphanEntryCited`, `LeakSupersedesNothing`, the four-barrier orphan
  worlds and their probes.
- **lean: the merge base lagged the pointer CAS across a container restart,
  and a repair re-cited a generation a peer had knowingly deleted** (H1c;
  L-107). The merge base was rewritten at step 7, after the CAS and the GC;
  a restart in that window left it a generation behind the document the
  workspace had installed, so an entry that install had cited read as a
  repair still owed, and after a peer's knowing delete of it (a
  collector-off store leaves the object) the repair re-cited it. The
  document's tombstone (next entry) closes it; a base-at-CAS arm written
  for this route was dropped as redundant once the tombstone was in — its
  known-bad world would not go red.
- **lean: a UI write adopted while still pending, cited by another writer
  and then knowingly deleted by it, was re-cited by the adopter's repair**
  (H1e, and H1d where the citer restarted before its window clear so the
  entry outlived the citation; L-108). The adopter's merge saw "baseline ≠
  merge base, object at the key" — the very view a delete that merely
  RACED the write leaves, where modify rightly wins — and nothing in the
  bucket told the two apart. The manifest now carries, per published
  delete, the etag it retired (`LeanManifest::tombstones`, inline in the
  single-object layout and one content-addressed object beside the chunks
  in the chunked one, named by the pointer and kept by the chunk reaper),
  until the path is cited again or 10,000 generations pass; a citation
  repair whose generation the tombstone names is void, and the path is
  queued as a tombstone for the tree. `ManifestTombstones` in the model;
  the world that fails without it, three tests that fail without the
  check.
- **lean: a queued deletion superseded by a re-cite settled without
  restoring the merge base, and when the path was deleted again before the
  install a clean copy of the retired generation stayed in the tree
  forever** (H1f; L-109; found by the model in the four-barrier orphan
  world once every other arm was on — the evening's one route that is not
  a resurrection, and the new `Inv_TreesConverged` is what saw it). The
  pull a superseded tombstone defers to is the next install's diff from
  the merge base, and the base had moved past the path when the deletion
  was queued; a second delete before that install left nothing to diff,
  the copy stayed uncited and was a citation-repair candidate at every
  barrier, so the barrier's skip-on-no-diff never fired either.
  Superseding a deletion now restores the merge base to the generation
  the delete retired (`ForeignChange.retired`); the next install queues
  the deletion again, or the upsert if the re-cite still stands.
  `SupersedeRestoresBase` in the model, the world that fails without it
  in 30 steps, its probe, and the test that fails with the one line
  reverted. The restore also showed `Inv_TreesConverged` a blind spot: with
  it in, the shipped leak rule flaps (the deletion is queued again at every
  install and superseded again) instead of settling, and "a queued deletion
  is pending work" excused that; the exception now holds only for a
  deletion the next consume would apply. And the restore closes H1's
  resurrection half on its own (an honest base leaves no repair to ride),
  as the tombstone does, so H1's known-bad world turns all four rules off
  and the collector-off leak worlds pin what each still carries, one arm
  apart: the retired-etag rule and the restore each close the resurrection
  alone; only the leak rule converges the tree (`LeakFlaps`, the restore
  alone, flaps forever; `LeakSkippedGeneration` shows the retired-etag rule
  cannot recognise a leak of a generation the tree never installed).
  The crash world at `OrphanTrack = TRUE` (`lean/SAFETY.md` §4.4's open
  half) then ran on the box to depth 25 and 149M distinct states without
...
- **lean formal: the gate is 136 runs** (117 + the nineteen of 2026-09-18),
  ...
  §4.11-12, review 2026-09-18 H8/H9). Two runs the killed local gate never
  reached were red on the Linux box: the tombstone-over-UI-write mutation
  and a trace control turned the tombstone key rule off while IMPL carried
  the retired-etag rule, which assumes it; both now turn it off together.
...
- **lean on Ozone 2.2.x: one writer per workspace.** Ozone 2.2.1 ignores
  ...
  second writer's garbage collector can delete the first writer's upload.
  S3 enforces it. The syncer does not refuse the configuration yet.
- **lean: a writer lost for good between an upload and its commit** (pod
  replaced, node gone) leaves an uncited object: the manifest, a fresh
  checkout and the live trees disagree about that path until some writer
  rewrites it. No acknowledged write is lost. Pinned by an ignored test.
- **lean: a writer that supersedes another writer's not-yet-cited upload**
  leaves that writer's commit citing a generation the key no longer holds
  until the superseding writer commits (the bytes are preserved).
- **lean gateway: a UI write can wait up to the window deadline (180 s)**
  behind a barrier that died mid-window before it answers.
- **lean formal gate:** `LeanBarrierLeaseSentinel` still violates
  `Inv_AckBoundaryCoherent` at depth 19. The code defect its trace exposed
  is fixed in this release (`remote.seq`); the invariant's one-directional
  refinement is not.
...
- **lean: a checkout can be SCOPED to part of the workspace.**
  `FLINT_SYNC_CHECKOUT_SCOPE` takes a comma list, and `checkout`
  materialises only the citations it admits — 3 files of 2001 on the
  drill, 22x faster than the whole tree. The admission filter in
  `checkout` is the WHOLE safety argument: a path this checkout never
  materialised is never cited, so `classify` can never read it as
  absent and the barrier can never DELETE its object. No scope filter
  is needed in the barrier, the sync or the delete sites, and four
  copies of a rule are four places for it to drift. Two constraints a
  reader must know before using it. The scope matches path
  **components**, not string prefixes — `f-00` does not admit
  `f-0001.txt` — so a flat directory of many files cannot be split by
  it. And `inst_base` stays the WHOLE manifest: narrowing it would make
  every unadmitted citation read as foreign, queue into the inbox, and
  land in the tree at the next barrier, which is a scoped checkout that
  downloads everything one barrier later. Design of record:
  `docs/plans/flint-lean-scoped-read-design.md`.
- **lean: `rescope` narrows or widens a live workspace's held set.**
  `flint-sync rescope inputs docs/spec.md`, or `rescope --all` for the
  whole tree — spelled out, because a bare `rescope` meaning
  "everything" would silently refetch a deliberately-scoped workspace's
  entire manifest by accident. A narrow is an **unwatch**, never an
  absence: the drop set is RECORDED in a replayable intent rather than
  re-derived, because re-deriving from the baseline loses the set the
  moment the uncite lands and re-deriving from the tree cannot tell a
  leftover from a file the agent created. A path with unpublished
  changes is refused at the door and KEPT on replay — two postures,
  deliberately, because a replay must converge and a door need not.
  Machine-checked in TLA+: `Inv_NarrowNeverDeletes` and
  `Inv_NarrowNeverRecites` both catch a deliberately-wrong narrow
  before they pass a right one.
- **lean: drafts — a durable edit that is NOT published.** An edit
  lands under `LEAN_DIR` rather than in the tree, which IS the safety
  argument: the scan cannot see it, so no barrier can publish it. The
  base etag is RECORDED, not enforced, and the resume view marks a
  draft whose base has moved.
- **lean: a large object materialises as PARALLEL RANGES.** Off by
  default (`FLINT_SYNC_RANGE_GET_MIN_MB`), and the report counts how
  many objects actually took the path — on a cluster nothing else can
  tell "ranging did not help" from "ranging did not run", and a
  threshold that never fires reads exactly like an optimisation that
  does not work.
- **lean gateway: conditional writes.** The HITL door took any `PUT`;
  two browsers that each read v1 and then wrote both got `200` and the
  second silently won — and because the gateway appends an inbox entry,
  the loser's bytes were not even recoverable from a manifest. It now
  answers forge's taxonomy, so the three doors are one shape: `428` for
  an overwrite naming no version, `412` (with the current etag on the
  response) for a stale one, `400` for `If-Match` together with
  `If-None-Match: *`. Creating stays unconditioned.
- **store: a cross-key server-side copy, and `flint-sync probe-copy` to
  verify a bucket supports it.** The probe takes NO lease and touches
  no tree, so it runs against a workspace with a live syncer — a probe
  that had to depose one to answer "does this bucket support X?" would
  be unusable on exactly the workspaces anyone wants the answer for.
- **docs: a data-flow poster per front end**, plus a fifth that is a
  use case rather than a front end, bound as one book. Every SVG is
  generated by `diagrams.py`; `pdfinfo` is the spill oracle.
...
- **lean: `checkout` no longer takes the publish lease.** The epoch cell
  at `<prefix>/.flint/lean/epoch` arbitrates who may INSTALL a manifest.
  A checkout installs nothing, and was standing in that queue anyway.
  What the claim bought was a deadlock wearing mutual exclusion's
  clothes: the claim loop supersedes a foreign holder only after six
  observations in which its token has not advanced, and a live holder
  renews, so its token always advances. A checkout that met a running
  publisher waited for it; a publisher that met a checkout waited for
  the checkout — forge's export has carried a timeout for exactly this
  since it shipped. Worse, on a quiet cell the wait does not end in a
  wait: it ends in a READER DEPOSING A LIVE PUBLISHER, sixty seconds
  in. Mid-read safety was never the lease's job and has not changed —
  every fetch carries the cited etag as `If-Match`. Measured on
  i4i.2xlarge over 20,000 8 KiB files: four syncers reading disjoint
  subtrees of one prefix run **2.1x** faster than one with the claim
  gone, and **0.41x** — slower than a single syncer — with it.
  `barrier`, `sync`, `rescope` and `recover-staged` still claim.
  `rescope` writes nothing to the bucket either, but it rewrites the
  held set and leaves a replayable intent behind, so it stays fenced
  until someone has walked that crash matrix with two of them running.
  One window opens that was unreachable before: a publisher landing a
  new generation mid-checkout. The refusal waiting in it accused a
  stranger of writing the bucket; the reader now re-reads the pointer
  before that error leaves, and a seq that has advanced names the
  publisher and says to re-run.
- **lean: `FLINT_SYNC_FANOUT` now defaults to 128** (was 32), and
  uploads get their own `FLINT_SYNC_UPLOAD_FANOUT`, which keeps 32.
  Small-file checkout is bound by objects in flight, not by bytes — an
  8 KiB entry charges the 1-permit minimum out of 512, so the byte
  budget never binds and fanout alone sets the rate. 32 to 128 is 2.5x
  (1,277 to 3,204 files/s); 256 and 512 are no better. The large
  workloads were measured BEFORE this landed rather than after: on big
  and mixed it is inert, and peak RSS is flat, because a 1 GiB entry
  takes the ranged arm and charges 64 permits, capping objects in
  flight at 8 regardless of fanout. Uploads are split off because they
  are bounded ONLY by fanout — there is no byte gate on that side, so
  the raise would have been a straight 4x on bodies held in memory —
  and because the barrier's lease fence lands only between chunks of
  `fanout * 16`, so it would also have widened the unfenced window from
  512 files to 2048.
...
- **`agent-fleet-doc-drill.sh` gained L5b**, which asserts the mandate
  ...
  wrong.
- **lean: an UNREADABLE path was published as a deletion, and the object
  deleted.** `confirm_absences` is the oracle that promotes a first
  absence to a published deletion, and a published deletion removes the
  citation AND issues the object `DELETE`. It asked
  `symlink_metadata(..).is_err()`, so `EACCES` on a parent directory,
  `EIO`, `EMFILE` and `ELOOP` all read as "the agent deleted it". The
  walk asks the SAME syscall and fails closed. Two call sites of one
  syscall had opposite error policies, and the one that can destroy data
  was the one guessing — with no second tick to catch it on a declared
  boundary.
- **lean: a full disk lost a foreign write, and a typo'd scope became
  the whole tree.** Both are a failing path returning something LEGAL
  instead of an error. A write failure while consuming an inbox entry
  was recorded as a *containment refusal* and the entry dropped — so
  whoever read the conflict record went looking for a planted symlink
  that was never there, while the foreign bytes stayed in the bucket and
  the workspace never took them. And `Scope::new` silently drops entries
  that are malformed, too long, or past the cap, so three bad paths
  normalised to an EMPTY scope — which reads everywhere as "no
  restriction", i.e. maximum privilege from a typo. An all-rejected
  scope is now refused.
- **lean: every fetched byte was copied an extra time, and every write
  froze the fan-out.** `get_range` ended in `AggregatedBytes::into_bytes`,
  which for any multi-frame body — i.e. always — takes the general
  branch: one full-size allocation and one full-size copy per range on
  top of the frames the SDK already holds. And the ranged arm drained
  its fan-out on ONE task and called blocking `pwrite` inline, so
  writing one range stalled the FETCH of every other range in flight.
  Real S3 on i4i.large: 477-500 to **654-676 MiB/s (+37%)**. Against an
  in-memory S3 serving 5.5 GiB/s: 887-890 to 1304-1310, with cost per
  byte FALLING 1.86 to 1.42 CPU-s/GiB — faster AND cheaper, which is
  what distinguishes removing work from redistributing it. The tuning
  knobs stop mattering: shipped defaults now equal hand-tuned, where
  before it was 391 vs ~700. The whole-object arm had the same inline
  write and it is fixed too, which is where every small file goes.
- **lean: the publish read every file TWICE**, and a large object was
  ONE stream in both directions. Together with the read-path work above
  the publish drill measured **1.55-1.63x**.
- **lean: the conflict log was an unbounded append that a hot path
  parses WHOLE.** Nothing truncated it, and `load_conflicts` reads and
  parses the entire file twice per sync honor, again per status read and
  per scrape — so the cost of a standing condition landed on the READ
  path, growing forever. It rotates at 1 MiB, and the rotated generation
  is read FIRST: `honor_sync` counts the log before a sync and skips
  that many after, so a rotation that SHORTENED the list would make it
  skip past the records the sync just produced and ack "no conflicts".
- **lean operator: a parse failure was reported as "an older syncer".**
  The holder's observed-state echo was read with
  `serde_json::from_str(e).ok()`, and `.ok()` maps a parse FAILURE onto
  `None` — which reports as "the lease holder writes no echo, a syncer
  older than the boundary-verbs protocol". A malformed or
  schema-skewed echo, exactly what a version skew produces, surfaced as
  a benign old binary. Present-but-unparseable and absent are now
  distinct, and `LeaseEcho`'s undefaulted fields gained `#[serde(default)]`
  so a merely INCOMPLETE echo parses rather than vanishing. Three env
  vars left behind by the v1.45.0 webhook removal
  (`FLINT_LEAN_SIDECAR_IMAGE`, `FLINT_LEAN_WEBHOOK_SERVICE`,
  `FLINT_LEAN_WEBHOOK_NAMESPACE`) are gone: a leftover that READS AS
  OPERATIVE is worse than no setting, because the next reader changes it
  and waits for an effect that cannot come.
- **lean: the PodMonitor has selected nothing since v1.45.0.** It
  selects `chert.us/lean-workspace`, a label the TENANT put on its own
  pod to opt into webhook injection; delivery moved to the node plugin
  and the label went with the webhook. A selector matching zero pods is
  indistinguishable from a fleet with no workspaces running, so nothing
  said so.
- **lite (tier): an evicted file whose name went stale read as ZERO
  BYTES, forever.** A reconciler row's only handle on its file is the
  recorded name, and that name goes stale in two ways that are not the
  inode dying — a hard-linked pair losing the name the row was written
  under, and a `RENAME` that lands before the process dies with its
  event undrained.
- **lite + forge (X24): a wake stamp read as PRESENCE woke every parked
  server one pass after it parked.** `chert.us/requested-at` was asked
  two different questions in one function — "how old is it?" while
  running, "is the key there?" once down — with nothing bridging the
  readings but an invariant that forge does not have. So one pass after
  ANY real suspend, presence read a stamp older than the threshold (being
  older is exactly what let it suspend) as a standing wake request, and
  every repository woke itself forever, rebuilding its emptyDir and
  paying a full restore once per threshold. The door had the mirror bug
  and the two CANCELLED, which is why nothing looked wrong. Drilled
  9/1 broken, 17/0 fixed on forge and 13/0 on lite — and lite's flap
  ERASES ITS OWN EVIDENCE, so only a transition COUNT can see it.
- **pNFS: a `GETDEVICEINFO` that answered with nonsense.**
  `endpoint_to_uaddr` correctly returns `Err` for a host that cannot
  become the IPv4 octets an RFC 5665 universal address requires — and
  its one production caller threw the `Err` away, encoding `host:port`
  where a universal address belongs. The IPv6 `host:port` joiner that
  RFC 3986 §3.2.2 asks for is now wired up rather than shipped unused:
  `2001:db8::1:2049` is ambiguous, not merely ugly.
- **forge: a push at a standby said "os error 2".** The hook socket was
  created only after the claim and the restore, so a push landing on a
  server that was not the writer met `ECONNREFUSED` and the client was
  told "No such file or directory" — true, and naming nothing an
  operator can act on. The listener comes up early and names the posture
  that refused it.
- **lite: the published guide told readers to install an operator four
  releases old.**
- **s3.csi drill: the residency check was cluster-wide.** S16 drains ONE
  node and then asserted no worker pod remained anywhere. On kind those
  are the same number — the rig has one node, so the query and the claim
  agreed by accident. On a real cluster it reported residue for a worker
  pinned to its own tenant's node, which is the product working. Both
  counts are scoped to the drained node now.
