...
| X23 | **`upload-pack` silently ignored `--filter`**: `uploadpack.allowFilter` was never set, so a client asking for a blobless clone got the whole repository and a warning it does not read | a partial clone of a repository whose size is its blobs downloads all of them | **fixed 2026-09-06** — `uploadpack.allowFilter=true` in `init_bare`; found by `forge/e2e/gitqual`, whose first draft of the leg asserted only that the clone SUCCEEDED and passed while the filter was being ignored. The leg now asserts blobs are actually missing |
...
| X14 | **the door's wake bound is a constant and a restore is proportional to the repository.** 180 s at the door; the restore fetches every pack, then installs refs, then runs fsck, then reports serving; a challenger restores only after it claims, and `Recreate` hands the successor a fresh emptyDir | the 40 GiB drill repository restores in 139 s from the delete; the design's own arithmetic for a wake after an unclean death — 60 s of quiet polls plus that restore — is 199 s against 180 s, and git clients do not retry the 503; every roll is a full restore of unavailability | **half built 2026-09-06** (`follow.rs`, `log.rs`) — the waiting half. A server that is WAITING for another's lease brings the repository down and PROVES it while it waits, and follows the batch log for what lands after; when it claims, its restore fetches nothing and walks only the tips that moved. It warms only while the holder's token still moves (`quiet_polls == 0`): a challenger about to take over a dead server must race, not download. The proof is incremental on any warm disk, a container restart included — `fsck --connectivity-only` over everything becomes `rev-list --objects <new tips> --not <proved tips>`, and it lapses the moment a file the last proof walked is not there. Measured on a real S3 API (`forge/e2e/composition/c7-prewarm.sh`, results `forge/e2e/results/c7-prewarm-2026-09-06.log`): against its own control, 430.0 MiB and a full proof become 0 MiB and none, restore 845 → 232 ms, claim-to-serving 861 → 222 ms. **Still open: the serving half** — refs answered from the snapshot before the packs land (walgit's refs level), and the operator-side decision to run a standby at all, since `Recreate` with one replica leaves nobody waiting to warm |
| X15 | **no undo.** The snapshot is replaced in place, versioning is OFF (design §3), the bare repository has no reflog and lives in an emptyDir; `repack -a -d` drops unreachable objects and the sweep deletes the old packs after the grace | a force-push or a bad merge is unrecoverable at the storage layer within at most `repack_threshold` (24) pushes; Continuity keeps every state, walgit retains superseded packs for a provenance window | **built 2026-09-06** (`undo.rs`) — a batch that would make a state unreachable writes an immutable copy of the snapshot it replaces at `git/undo/<seq>.json` BEFORE its CAS, and both sweeps treat the packs a copy names as referenced until `undo_window_secs` (7 days) has passed. Only a delete or a non-fast-forward move pays: a fast-forward loses nothing, so an ordinary push's fixed cost is unchanged. `--undo-list [<ref>]` reads the points; putting one back stays an operator's decision. **The reader-catch-up half was built 2026-09-06** (`log.rs`): every snapshot CAS also puts one small immutable object at `git/log/<seq>.json` naming the refs that moved, the packs that appeared with the files beside them, and the packs that left. It is written AFTER its CAS, so a fenced batch can never leave an entry for a push that did not land; a follower advances only along a contiguous chain and a hole sends it back to the snapshot; nothing in the log is a reference, so the sweep is unchanged. An idle follower's poll is one 404 on `<seq+1>` instead of a whole ref map. It costs the batch exactly one PUT — the fixed per-push cost is 4 requests with the log off and 5 with it on, pinned by `the_batch_log_costs_exactly_one_put_per_batch` — and no latency, because it rides beside the derived files rather than after them. Bounded by count (`log_max_entries`, 512) and pruned in the sweep; `--log-list [<n>]` reads it back. 0 turns it off and is the control arm |
...
| X18 | **compaction has no tiers.** Past `repack_threshold` a full `repack -a -d -b` re-uploads the whole repository (measured `22807f9b`; 33× the bytes and an 816 s push on the wire, §9.1) | cost proportional to the repository every 24 pushes | **designed and built 2026-09-06** — `docs/plans/forge-compaction-tiers-design.md` (three candidates, six refutations, candidate C: geometric folds of plain packs by bytes beside the loop, one bitmap on the base, nothing new in the bucket); `forge/syncer/src/fold.rs`; `FLINT_FORGE_FOLD_FACTOR=0` kept the old rule as the control until the re-match (§9.1) had run, and **the control was deleted 2026-09-07** — it was a third un-renewed CAS site with no coverage check (audit F3), and factor 0 now means no compaction at all |
| X19 | **the snapshot carries a map of every ref** and is rewritten by every batch | at 466 k refs (walgit's reference monorepo) tens of MB per CAS and per restore; under 1 MB at agent-fleet ref counts | **measured 2026-09-06** (`forge/e2e/refscale/`, results `forge/e2e/results/refscale-2026-09-06.log`) — this is what the rate leg on runcc saw: 17.4 → 9.7 pushes/s over three minutes as the branch count grew ~900/min. A lone push that moves ONE ref costs 240 ms at 0 refs and 1,824 ms at 8,000. **But the control refuted the obvious reading**: the identical push to a plain bare repository with the same refs and no forge costs 79 → 1,147 ms, so 63 % of the growth is `receive-pack` advertising every ref to the client and NO git server escapes it — walgit's own receive-pack included. forge's share was 677 ms at 8,000 refs, of which three of the four O(refs) costs were the DUMB protocol's: `update-server-info` over every ref (220 ms) and `info/refs` uploaded whole (511 KB), for a view nothing on the serving path reads. **Half built**: those are on a timer now (`FLINT_FORGE_DERIVED_EVERY_SECS`, default 60; 0 is the old behaviour and the rig's control arm), taking forge's share to 385 ms — 43 % of it, 20 % of the whole push — and the fixed per-push request count from 4 to 2 for every batch but one a minute. **Still open**: the snapshot's ref map itself (545 KB at 8,000 refs, rewritten by every batch) and the `for-each-ref` the batch's agreed view runs over every ref; the map's answer is to shard it into content-named objects the snapshot names, so a batch rewrites only the shards it touched |
...
| X24 | **the wake stamp is never cleared and the Suspended branch tests its PRESENCE.** `reconcile.rs` keeps `chert.us/requested-at` deliberately ("the door's heartbeat"; lite clears it on Active, `lite_operator/reconcile.rs:1786-1791`), while `idle::decide` reads it two ways in one function: age in the running branch (`idle.rs:147-151`) and `contains_key` in the suspended branch (`idle.rs:114-121`). One pass after any real suspend a repository is `Suspended` carrying a stamp older than its threshold — that is what let it suspend — so presence reads it as a standing request and `replicas_for(IdleSuspended, Wake)` returns 1. The door's half is the same bug pointing the other way: `ShareView::of_repo`'s `wake_requested` is presence (`resolve.rs:223-228`) and gates the re-arm at `git.rs:605`, so the two cancel today — fix either alone and the repository either flaps or can never be woken again | after its FIRST door wake a repository can never stay parked: it scales to zero and returns on the next pass, rebuilding the `emptyDir` and paying a full restore, once per threshold forever. No durability consequence; the ladder saves nothing and costs pod churn | **found + fixed 2026-09-10.** Both halves ask one shared question, `clock::request_is_live` (`lite_operator/idle.rs`), against the repository's own threshold. Three unit tests, each of which FAILS against the old rule and was watched to fail before the fix; both halves mutation-checked line by line. Second change in the same commit: the ladder turned OFF while a repository is parked now RAISES it — with no threshold, `Hold` meant down forever with no field that brings it back. **Why nothing caught it:** the pre-existing test `a_suspended_repository_wakes_on_a_request_and_holds_without_one` compares a one-second-old stamp against NO stamp — two dimensions at once, so it passes under either rule; F12's and F13's P6 each park a repository ONCE, on one the door had never woken (so no stamp exists), and F13 then deletes the annotation by hand before its wake; the EC2 acceptance run of 2026-09-04 did the same by accident. Drilled by `forge/e2e/f17-ladder.sh`, which runs TWO park cycles, writes no annotation, and samples the second park rather than reading it once |
