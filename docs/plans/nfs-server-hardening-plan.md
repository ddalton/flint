...
All verified in code at `50e5cd8`, per-binary on the MDS path:
...
| Namespace-op durability (DUR-1) | 🔶 **fix in flight** — uncommitted `metadata_sync.rs` in the working tree: parent-dir fsync on CREATE/REMOVE/RENAME/LINK, on by default, fsync failure → NFS4ERR_IO | leg F4 is its verification; land before GA |
