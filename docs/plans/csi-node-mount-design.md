...
**Termination ordering: no PodDisruptionBudget** (user question,
2026-09-03: *"so do we really need a PDB … I don't like PDBs. So can
you check the literature on this? Is there a better solution?"*). §3.6
and §5 specified a `PodDisruptionBudget` with
`minAvailable: <huge integer>` over the workers — a "never voluntarily
evict" written as a budget, because bare pods may use neither
`maxUnavailable` nor a percentage. It was built, and then removed. The
reason is that a worker and its tenant can be separated on three paths,
and the budget only ever touched one:

| path | mechanism | what a budget does |
|---|---|---|
| `kubectl drain`, autoscaler scale-down | eviction API | refuses — the budget applies |
| node reboot, **spot reclamation**, cordon+shutdown | kubelet graceful shutdown | nothing; the eviction API is not involved |
| kernel panic, power loss, SIGKILL | nothing | nothing |

On a pure-spot fleet the middle row is the *ordinary* case. A budget is
inert there, so it cannot be the design's answer to "a worker is never
taken away from a tenant still using its mount"; it also contradicted
the workers' own
`cluster-autoscaler.kubernetes.io/safe-to-evict: "true"` (a budget that
refuses every eviction and an annotation inviting one resolve, in the
autoscaler, as "do not scale this node down"), and it blocked a drain
for as long as any tenant declined to terminate — at which point the
operator reaches for `--disable-eviction` and loses the protection
entirely. The two upstream drivers with this exact architecture — a
per-volume mount pod in a system namespace — hit the same failure and
answered it with ordering, not a budget:
[awslabs/mountpoint-s3-csi-driver#607] *"Draining nodes breaks
applications depending on the mountpoint volume while draining"* was
answered by graceful eviction, in which a mount pod stays alive until
every workload pod using its volume has terminated; and
[juicedata/juicefs-csi-driver#856] *"mount pod terminates before my
application pod when draining a node"* is the same report, from a
node-termination handler draining an AWS **spot** node. Istio's CNI node
agent likewise runs as a DaemonSet with a PriorityClass and no
per-node-agent budget.

What is built instead is one mechanism per path that HAS one:

1. **PriorityClass `flint-s3-worker`** (`value: 100000`,
   `preemptionPolicy: Never`, chart-created, `workers.priorityClassName`)
   — kubelet's graceful-shutdown manager terminates pods by priority,
   lowest first (`shutdownGracePeriodByPodPriority`), so a worker
   ranked above its tenants is torn down after they have stopped
   writing. 100000 sits far below `system-cluster-critical`
   (2000000000) so the plugin, CNI and kube-proxy still outrank it, and
   far above the 0 an unclassed tenant carries. `preemptionPolicy:
   Never` so a worker never evicts a tenant to schedule itself.
2. **A `preStop` hook on every worker** —
   `flint-s3-worker await-release`, a new subcommand that waits for the
   `released` marker `NodeUnpublishVolume` writes into the worker's
   `comm` emptyDir. Kubelet runs `preStop` *before* the SIGTERM, so an
   evicted or drained worker keeps serving its tenant's mount until the
   volume is genuinely released. The eviction is **accepted** — nothing
   is refused, nothing retries, the drain converges — it simply does
   not take the mount with it. The budget is `workers.prestopSecs`
   (default 60), **added to** `terminationGracePeriodSeconds` rather
   than carved out of it, so a lean syncer's final publish keeps its
   full drain budget; the hook exits 0 when the budget expires, so it
   can never wedge a node. On the ordinary unpublish path the marker is
   already there and the hook costs one `stat`.
3. **Detection, which covers all three paths** — `NodeUnpublish` asks
   `worker::is_gone` first and emits `SyncerGoneAtUnpublish` on the
   tenant pod naming `recover-staged`, so a worker lost to a panic
   leaves staged-but-uncited objects recoverable rather than silent.
   The budget only ever upgraded "detected and recoverable" to "did not
   happen", on the one path a spot fleet is least likely to take.

The `safe-to-evict: "true"` annotation and the Node `ownerReference`
with `controller: true` both stay, and the annotation is now
load-bearing: a Node ownerReference is not a controller kind the
autoscaler recognises, so a bare worker without it blocks scale-down
outright. S16 asserts the whole shape, including the *absence* of a
budget in the workers namespace — a budget that crept back would make
that leg pass for the wrong reason. The broker keeps its PDB: it is a
Deployment with replicas, `minAvailable: 1`, rendered only when
...
A pod carries the label `chert.us/passthrough-mount: <cr-name>`
...
Same admission shape (`objectSelector: chert.us/lean-workspace Exists`,
    ...
    - name: data
      csi:
        ...
        volumeAttributes:
          ...
          # chert.us/workspace: team-a         # or a FlintLeanWorkspace (lean arm)
          # chert.us/uid: "1001"               # optional presentation override (§3.5)
...
`chert.us/mount` / `chert.us/workspace` is accepted; `chert.us/uid` and
`chert.us/gid` are accepted as narrowing presentation overrides; every
...
today mounts as its own uid must say so in `chert.us/uid` or the CR, else
...
NodePublishVolume(req):
  1  parse: selector (exactly one of chert.us/mount|workspace), narrowing attrs, readonly,
  ...
  6  create-or-adopt worker pod (label chert.us/volume-id=<vid>, nodeName=<this node>, image chart-pinned,
...
NodeUnpublishVolume(volume_id, target):            # only these two fields arrive (csi.proto:1513,1523)
  ...
  4  write <comm>/released — the worker's preStop hook stops waiting (§0);  delete worker pod (direct delete: the eviction
       API, and therefore any budget, is never consulted);  rm -rf <plugin>/volumes/<vid>;  OK
```
...
| **Drain / eviction** | pod SIGTERM → drain in the pod's grace | tenant eviction is a normal termination → `NodeUnpublish` → drain with its own grace. `kubectl drain` evicts tenants and workers alike; a worker's eviction is **accepted**, and its `preStop` hook (`flint-s3-worker await-release`, bounded by `workers.prestopSecs` and ADDED to the grace) holds the container open until `NodeUnpublish` writes the `released` marker — so the mount outlives the eviction decision by exactly as long as the tenant needs it, and the drain still converges; the plugin deletes workers directly (a direct delete never reaches the eviction API); `priorityClassName` orders the kubelet's own graceful shutdown so tenants terminate first; workers carry `cluster-autoscaler.kubernetes.io/safe-to-evict: "true"` and an `ownerReference` to the Node with `controller: true` (so `kubectl drain` treats them as managed and waits on the eviction API instead of refusing controller-less pods without `--force`, `kubectl/pkg/drain/filters.go:236-249`) so a vanished node GC's them. Spot reclaim: no eviction API is involved at all — the ordering there is the PriorityClass, and a worker that dies regardless is *detected* at unpublish (`SyncerGoneAtUnpublish` naming `recover-staged`), which is the protection that covers all three paths | same |
...
| **Metrics** | `0.0.0.0:<port>` in the pod network; `PodMonitor` by label; per-namespace NetworkPolicy (`flint_sync.rs:388-440`; `monitoring.yaml:10-65`) | the syncer pod carries the port and `chert.us/tenant-namespace`/`chert.us/workspace` labels; the `PodMonitor` and NetworkPolicy move to `flint-workers` (one place) | moves |
...
its node (`chert.us/node=<name>`), re-arms liveness probes and resumes
...
- **Admission-time UX.** A typo in `chert.us/mount` is a pod stuck in
...
| 14 | ~~CA / Karpenter treatment of bare worker pods with `safe-to-evict: true` + PDB + Node ownerReference~~ **RESOLVED 2026-09-03 by removing the PDB** (§0): a budget refusing every eviction and a `safe-to-evict: "true"` inviting one are a contradiction the autoscaler resolves by not scaling the node down. The annotation stays and is now load-bearing — a Node `ownerReference` is not a controller kind the CA recognises, so without it scale-down is blocked outright. Ordering moved to the PriorityClass and the `preStop` hook | scale-down behaviour | one scale-down leg on a runb* cluster is still owed; kind cannot exercise an autoscaler |
...
(CSIDriver, DaemonSet, `flint-workers` namespace + VAP + worker PriorityClass, broker
...
§5). *That budget was then removed outright* (2026-09-03, §0
*Termination ordering*): it covered one of three termination paths, and
not the one a pure-spot fleet takes, while contradicting the workers'
own `safe-to-evict`. The `controller: true` ownerReference and the
`safe-to-evict` annotation both stay; ordering is now a PriorityClass
plus a worker `preStop` hook, and S16 asserts the *absence* of a
budget. T2 "closed" was over-claimed: any process under an entitled SA can
