...
| B1 | **PASS** | Three arms. Liveness first: untouched, the share reached `IdleSuspended` at t+139s with `replicas=0`, PVC still Bound (same UID), state carried in `chert.us/idle-state`/`idle-since` annotations, `IdleSuspended` event emitted. Arm 2 — 170s of `/status` polling only (**11 × HTTP 200**, so the poller provably reached the hub): still suspended. Arm 3 — 170s of authenticated `/files` calls (**6 × 200**): stayed `Ready`/`replicas=1`. Same duration, same rig, only the endpoint differs. `/status` is not activity; the file API is. |
...
10. **Removing `spec.idle` does not un-park a parked share.** With the policy
    ...
    `chert.us/idle-state` annotation — `IdleEligible=False (Held) idle and
...
- The `chert.us/idle-state` annotation reads `Suspended` while `status.phase`
